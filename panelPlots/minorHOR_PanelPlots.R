...
cen = read_tsv(paste0(dat, "/annotations/t2t_cenAnnotation.v2.021621FORMATTED.bed"), col_names = F) %>%
  ...
  meth <- ggplot(all.dat, aes(x = (start/1e6), y= smooth))+geom_line(size =1) + labs(y="Methylation")+theme_classic(base_size = 10)+xlim((rstart/1e6),(rend/1e6))+theme(axis.title.x=element_blank(),axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.line.x = element_blank())+theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))+ylim(0,1)+geom_vline(xintercept = 17.493880, linetype="dashed")+geom_vline(xintercept = 17.556302, linetype="dashed")
 
  #17493880-17556302
   cov <- ggplot(all.dat, aes(x = (start/1e6), y= ont_cov))+geom_histogram(stat = "identity") + labs(y="Coverage", x="Genomic Coordinates (Mb)")+theme_classic(base_size = 10)+xlim((rstart/1e6),(rend/1e6))+theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  ...
  top_row <- plot_grid(meth,cov,ncol = 1, align="v",rel_heights = c(3/4,1/4))
  top_row
  ggsave(
    paste0(figs, "/methyl_profiles/", chrom,"minor_HOR_PanelPlotv2Cov.pdf"),
    plot = top_row,
    scale = 1,
    width = 10,
    height = 3

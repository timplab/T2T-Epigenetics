...
regs <- read_tsv(paste0(dat, "/TE/TeloComposites/TELO-composite-unitA_RM-polish_APR2121_SHoyt.txt"), col_names = T) %>%
...
chm13.ovl.labs <- chm13.ovl %>%
  ...
  ungroup() %>%
  group_by(`49bp_yes/no`) %>%
  arrange(min) %>%
  mutate(med_smooth = rollmean(med, 10, NA))

# plot -- include annotation for number of repeats in each group
p <- ggplot(chm13.ovl.labs,aes( x = min, y = med_smooth, color=`49bp_yes/no`, fill=`49bp_yes/no`), alpha=.5)+
...
geom_text(data = chm13.ovl %>% 
              group_by(`49bp_yes/no`) %>%
              select(`49bp_yes/no`,gene_start) %>%
              distinct() %>%
              summarise(n_genes=n()) %>%
              distinct() %>%
              mutate(height=case_when(`49bp_yes/no` == "yes" ~ 0, 
                          `49bp_yes/no` == "no" ~ .1, 
                          TRUE ~ .02)),
            aes(x=1,
                y=.85+height,

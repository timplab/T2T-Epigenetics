...
freqplot <- ggplot(chm13.ovl.labs,aes(x=min,y=med_smooth))+theme(legend.position = "left", legend.direction="vertical",axis.title.x=element_blank())+scale_x_continuous(breaks= c(-1,0,bodylen,bodylen + 1), labels = c(paste0("-",flankn/1e3,"kb"),"TTS","TES",paste0("+",flankn/1e3,"kb")))+geom_smooth(method="loess",se=F, span=.1)+ylim(0,1)
#+geom_line()#

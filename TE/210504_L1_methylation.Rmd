...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","L1H_hot_not_1.25kb.pdf"),
  plot = p,
  scale = 1,
  width = 15,
  height = 10,
...
#  geom_smooth(se=T)+lims( y = c(0,1)) +
 # geom_ribbon(aes(ymin=bot, ymax=top), alpha=0.2, size=0)+
  ...
  labs( x = "Genomic Position", y = "Aggregated Methylation Frequency") +
theme(legend.background = element_rect(color = "black")) +
  theme_classic()+scale_x_continuous(breaks= c(-1,0,bodylen,bodylen + 1), labels = c(paste0("-",flankn/1e3,"kb"),"Start","End",paste0("+",flankn/1e3,"kb")))+facet_wrap(~censat_name,ncol=1)
p
...
ggplot(ovl, aes(x=name, y=methylated_frequency, fill=censat_name))+geom_violin(position = position_dodge())+geom_boxplot(width=.1,position = position_dodge(width =0.9))+facet_wrap(~HOT)

ggsave(
  paste0(dat, "/figures/evol_meth/TE/L1_hotvsnot_violin.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 10,
  height = 5,

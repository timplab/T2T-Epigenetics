...
reps <- read_tsv(paste0(dat, "/TE/beds/SVA-Retroposon_chm13v1_polish-033121_HOT-syn-CEN-subTELO-cenSAT-CBmotif-avgMeth_V3.bed"), col_names = c("chrom", "start", "end", "length", "query", "strand", "rep_type", "rep_class_rep_family", "position_begin", "in_end", "repeat_left", "SW", "perc_div", "perc_del", "perc_ins", "HOT", "no_syn", "CEN", "subtelo", "cenSAT", "CENPB_pJalpha", "average_methylation_frequency")) %>%
...
saveRDS(cpg.dist, paste0(dat, "/TE/SVA_chm13v1_polish-033121_CpG_dist100bp.rds"))
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_100bp.pdf"),
  plot = plot1,
  scale = 1,
  width = 15,
  height = 10,
...
ovl <- FindOvls(t2t.cpg.loci, GRanges(reps)) %>%
  ...
  group_by(ID) %>%
  summarise(num_cpg=n()/length, rep_type=rep_type,average_methylation_frequency=average_methylation_frequency) %>%
  distinct()

my_comparisons <- list( c("SVA_A", "SVA_B"), c("SVA_A", "SVA_C"), c("SVA_A", "SVA_D"), c("SVA_A", "SVA_E"), c("SVA_A", "SVA_F"),  c("SVA_B", "SVA_C"), c("SVA_B", "SVA_D"),  c("SVA_B", "SVA_F"),  c("SVA_B", "SVA_F"),  c("SVA_C", "SVA_D"),  c("SVA_C", "SVA_E"), c("SVA_C", "SVA_F"), c("SVA_D", "SVA_E"),  c("SVA_D", "SVA_F"),  c("SVA_E", "SVA_F"))
ggplot(ovl, aes(x=rep_type, y=num_cpg, fill=rep_type))+geom_boxplot()+ stat_compare_means(comparisons = my_comparisons)

ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_CpG_KWtest.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,
...
ggplot(ovl, aes(x=rep_type, y=average_methylation_frequency, fill=rep_type))+geom_boxplot()+ stat_compare_means(comparisons = my_comparisons)

ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_averagemethylation_KWtest.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_hot_not_100bp.pdf"),
  plot = plot1,
  scale = 1,
  width = 15,
  height = 10,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_hotvsnot_CpG_KWtest.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,
...
ggplot(ovl, aes(x=rep_type, y=average_methylation_frequency, fill=HOT))+geom_boxplot()+ stat_compare_means(method = "kruskal.test")

ggsave(
  paste0(dat, "/figures/evol_meth/TE/","SVA_A-F_average_methylation_frequency_KWtest.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,

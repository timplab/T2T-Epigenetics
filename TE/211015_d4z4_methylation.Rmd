...
```{r}
regs <- read_tblout(paste0(dat, "/revision_analysis/satellites/d4z4/d4z4_hmmsearch.tsv")) %>%
...
region=data.frame(chr="chr4", start=193492413, end=193540000) %>%
  GRanges()
...
arrow <- ggplot()+ geom_segment(data = regs, aes(x = start, xend = end, y = 0, yend = 0), 
               colour = "black", 
...
ggsave(
  paste0(figs,"CHM13_d4z4_Methylation.pdf"),
  plot = p,
  scale = 1,
  width = 10,
  height = 4,

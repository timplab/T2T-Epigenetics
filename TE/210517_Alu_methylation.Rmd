...
reps <- read_tsv(paste0(dat, "/TE/beds/Alu-SINE_chm13v1_polish-033121_HOT-syn-CEN-subTELO-cenSAT-CBmotif-avgMeth_V3.bed"), col_names = c("chrom", "start", "end", "length", "query", "strand", "rep_type", "rep_class_rep_family", "position_begin", "in_end", "repeat_left", "SW", "perc_div", "perc_del", "perc_ins", "HOT", "no_syn", "CEN", "subtelo", "cenSAT", "CENPB_pJalpha", "average_methylation_frequency")) %>%
...
my_comparisons <- list( c("AluJ", "AluS"), c("AluJ", "AluY"), c("AluS", "AluY") )
ggplot(ovl, aes(x=name, y=num_cpg, fill=name))+geom_boxplot()+ stat_compare_means(comparisons = my_comparisons)

ggsave(
  paste0(dat, "/figures/evol_meth/TE/","Alu_YvsSvsJ_CpG_KWtestV3.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","Alu_YvsSvsJ_average_methylation_frequency_KWtestV3.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 5,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","Alu_hot_not_1.25kbV3.pdf"),
  plot = p,
  scale = 1,
  width = 15,
  height = 10,
...
reps_sub <- reps %>%
  ...
  mutate(censat_name = ifelse(grepl("censat", censat_name), ".", censat_name)) %>%
  mutate(censat_name = ifelse(grepl("rDNA", censat_name), "rDNA", censat_name)) %>%
  mutate(censat_name = ifelse(grepl("gsat", censat_name), "GSat", censat_name)) %>%
  filter(censat_name != "rDNA") %>%
  filter(censat_name != "GSat") 
library(ggpubr)
...
p <- ggplot(ovl, aes(x=name, y=methylated_frequency, fill=censat_name))+geom_violin(position = position_dodge())+geom_boxplot(width=.1,position = position_dodge(width =.9))+facet_wrap(~HOT)

ggsave(
  paste0(dat, "/figures/evol_meth/TE/Alu_hotvsnot_violinV3.pdf"),
  plot = p,
  scale = 1,
  width = 10,
  height = 5,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","Alu_region_hotvsnotV3.pdf"),
  plot = q,
  scale = 1,
  width = 10,
  height = 10,
...
ggsave(
  paste0(dat, "/figures/evol_meth/TE/","Alu_hot_not_regions_1kbV3.pdf"),
  plot = p,
  scale = 1,
  width = 15,
  height = 10,

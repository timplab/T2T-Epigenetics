...
#chm13_meth$methylated_frequency <- chm13_meth$score

chm13_meth <- read_tsv("/kyber/Data/Nanopore/Analysis/gmoney/CHM13/v1.0_final_assembly/revision_analysis/HG002_pooled/HG002_CpG_methylationFrequency_pooled.tsv") 

# load rep file
reps <- read_tsv(paste0(dat, "/revision_analysis/satellites/ACRO/annotate_units/", "ACRO_subunits_mergedStranded_withArray.bed"), col_names = c("chr","start", "end", "strand", "chr_1", "array_start", "array_end", "len", "status")) %>%
...
freqplot <- ggplot(chm13.ovl.labs,aes(x=min,y=med_smooth))+theme(legend.position = "left", legend.direction="vertical",axis.title.x=element_blank())+scale_x_continuous(breaks= c(-1,0,bodylen,bodylen + 1), labels = c(paste0("-",flankn/1e3,"kb"),"TTS","TES",paste0("+",flankn/1e3,"kb")))+geom_line()#+ geom_smooth(method="loess",se=F, span=.1)+ylim(0,1)
#+geom_line()#

# plot heatmap
plot <- ggplot(chm13.ovl,aes(x=dist,y=-ID,fill=methylated_frequency))+ geom_tile()+scale_fill_gradient(low = "blue", high = "red", na.value = NA, limits=c(0,1))+scale_x_continuous(breaks= c(-1,0,bodylen,bodylen + 1), labels = c(paste0("-",flankn/1e3,"kb"),"TTS","TES",paste0("+",flankn/1e3,"kb")))+theme(legend.position = "bottom", legend.direction="horizontal")+theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title.x=element_blank(),axis.title.y=element_blank())

# stack density and heatmap
plot_grid(freqplot, plot, ncol=1, align = "v", rel_heights=c(1/3,1))

ggsave(
  paste0(dat, "/figures/evol_meth/TE/ACRO-CHM13CpG_deeptoolsHeatmap.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 5,
  height = 12,

...
all_marks <- read_delim("/kyber/Data/Nanopore/Analysis/gmoney/CHM13/v1.0_final_assembly/encode/encode_macs2_peaks_all_NBPF_intersections/num_NBPF_peaks.txt", delim = "\t", col_names = F) %>%
...
ggsave(filename = paste0(figs, "/ENCODE_NBPF_allPeaksBar.pdf"), 
       plot = last_plot(),
       width = 8,
       height = 5)
...
all_marks <- read_delim("/kyber/Data/Nanopore/Analysis/gmoney/CHM13/v1.0_final_assembly/encode/encode_macs2_peaks_all_NBPF_intersections/num_NBPF_peaks.txt", delim = "\t", col_names = F) %>%
  ...
  mutate_if(is.character, str_trim) %>%
  separate(col = mark,into = c("mark"),sep = "\\.") %>%
  filter(mark %in% c('H3K36me3','H3K27me3')) %>%
  mutate(num=as.numeric(num)) 


all_marks %>%
  filter(cell_line != "RWPE2") %>%
  group_by(cell_line) %>%
  mutate(total=sum(num), frac=num/total) %>%
ggplot(aes(y=frac, x=cell_line, fill=mark))+geom_bar(stat="identity", alpha=.7)+ theme(text = element_text(size=20))+theme_classic()+labs(x="Cell Line", y = "Number of MACs Peaks")


ggsave(filename = paste0(figs, "/ENCODE_NBPF_StackedBar.pdf"), 
       plot = last_plot(),
       width = 8,
       height = 5)
